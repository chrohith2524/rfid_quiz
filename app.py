...
<meta charset="utf-8"><title>RFID Quiz (Online)</title>
<style>
...
function flash(ok){
  ...
  stg.classList.remove('ok-flash','wrong-flash');
  void stg.offsetWidth;
  stg.classList.add(ok?'ok-flash':'wrong-flash');
}
...
s.on('update', d=>{
  ...
  if(d.stat==='ok')flash(true);
  if(d.stat==='wrong')flash(false);

  // ✅ Auto-clear status after 1.2 seconds
  if(d.stat==='ok' || d.stat==='wrong'){
    setTimeout(()=>{
      st.textContent='';
      st.className='neutral';
    },1200);
  }
});
</script></body></html>"""
