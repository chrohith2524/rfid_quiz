...
def resolve(uid, category):
    """Category-aware resolution so the same tag works in all modes."""
    if category == "Letters":
...
def finish_game():
    """Save once per game; push history and 'done' message."""
    if state["finished"]:
        return
    state["finished"] = True
...
table{margin:auto;width:80%;border-collapse:collapse;margin-top:20px;max-width:700px}
...
<h2>📊 Last 5 Games</h2>
<table>
  <thead><tr><th>Category</th><th>Score</th><th>Time (sec)</th></tr></thead>
  <tbody id=hist></tbody>
</table>
...
function phrase(cat,i){
  ...
  return i; // Shapes: say the shape name
}

function renderHistory(list){
  hist.innerHTML=list.map(x=>`<tr><td>${x.category}</td><td>${x.score}/${x.total}</td><td>${x.time}</td></tr>`).join('');
}
